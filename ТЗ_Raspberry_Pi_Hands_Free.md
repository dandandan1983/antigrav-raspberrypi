...
```mermaid
graph TB
    Phone[📱 Телефон] <-->|Bluetooth HSP/HFP| BT[Bluetooth менеджер]
    BT <--> Main[Главный модуль Python]
...
sequenceDiagram
    ...
    Phone->>BT: Запрос на подключение
    BT->>Main: Событие: новое подключение
    Main->>Audio: Настроить аудио маршрут
    Audio-->>Main: Готово
    Main->>BT: Принять подключение
    BT-->>Phone: Подключено
    
    Phone->>BT: Входящий вызов (AT команда)
    BT->>Call: Обработать входящий вызов
    Call->>Main: Событие: входящий вызов
    Main->>Call: Принять вызов
    Call->>BT: Отправить AT команду (ATA)
    BT-->>Phone: Вызов принят
    
    Main->>Audio: Начать аудио поток
    Audio-->Phone: Передача аудио (микрофон)
    Phone-->Audio: Получение аудио (динамики)
```
